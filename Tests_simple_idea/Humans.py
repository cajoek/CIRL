...
class Optimal:
    ...
    def act(self, state, theta):
        action_score = np.max(self.Q[state, :, :, theta], axis=1)
...
class Teaching:
    ...
    def act(self, state, theta):
        ...
        if np.count_nonzero(optimal_actions) > 1:
            optimal_actions[0] = 0  # Action 0 is move left, avoid this if possible
...
class Repeating:
    ...
    def act(self, state, theta):
        ...
        action_score = self.Q[state, :, last_aR, theta]
        action_prob = softmax(action_score, self.eta)
        aH = np.random.choice(action_prob.size, p=action_prob)
        return aH
    ...
    def act(self, state, theta):
        ...
        for aH in range(self.n_aH):
            for aR in range(self.n_aR):
                action_score[aH] = self.Q[state, aH, aR, theta] * action_freq[aR]/np.sum(action_freq)
